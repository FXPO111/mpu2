...
export async function proxyAuthGet(request: NextRequest, backendPath: string) {
  ...
  for (const baseUrl of resolveBackendCandidates()) {
    try {
      const resp = await fetchWithTimeout(`${baseUrl}${backendPath}`, { method: "GET", headers: { Authorization: `Bearer ${token}` }, cache: "no-store" }, FETCH_TIMEOUT_MS);
      return new NextResponse(await resp.text(), { status: resp.status, headers: { "content-type": resp.headers.get("content-type") ?? "application/json" } });
    } catch {}
  }
...
export async function proxyAuthPost(request: NextRequest, backendPath: string) {
  ...
  for (const baseUrl of resolveBackendCandidates()) {
    try {
      const resp = await fetchWithTimeout(`${baseUrl}${backendPath}`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify(payload), cache: "no-store" }, FETCH_TIMEOUT_MS);
      return new NextResponse(await resp.text(), { status: resp.status, headers: { "content-type": resp.headers.get("content-type") ?? "application/json" } });
    } catch {}
  }
  return NextResponse.json({ error: { message: "Backend unavailable" } }, { status: 502 });
